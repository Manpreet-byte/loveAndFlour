      ...
      compute: async () => {
        ...
        const [catRows] = await pool.query(
          `SELECT cc.course_id,
            ...
            WHERE cc.course_id IN (?) AND cat.type = 'course'
      ...
      compute: async () => {
        ...
        const [categoryRows] = await pool.query(
          `SELECT cc.category_id AS course_id, cat.slug AS course_slug, cat.name AS course_name
             FROM course_categories cc
             JOIN categories cat ON cat.id = cc.category_id
            WHERE cc.course_id = ? AND cat.type = 'course'
         ORDER BY cat.name ASC`,
          [row.id],
        );
  ...
  try {
    ...
    const payload = await cacheWrap({
      ...
      compute: async () => {
        const [rows] = await pool.query(
          type
            ? 'SELECT id, type, name, slug, description FROM categories WHERE type = ? ORDER BY name ASC'
            : 'SELECT id, type, name, slug, description FROM categories ORDER BY type, name ASC',
          type ? [type] : [],
        );
